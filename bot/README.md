...
- **Уведомления** — бот присылает напоминания за 2 дня, за 1 день и в день списания (ежедневная проверка в 9:00 по локальному времени). Для категории «на-вылет» приходит отдельное сообщение.
- **Список подписок** — команда `/list` выводит подписки по ближайшей дате списания. Если дата уже прошла, бот переносит её на следующий период (месяц/год) по той же логике, что и веб-приложение, и сохраняет в Firebase. Подписки помечаются: 🟧 нужные, 🔲 на-вылет.
...
- **Смена категории** — команда `/category` меняет статус подписки (🟧 нужная ↔ 🔲 на-вылет) через кнопки, либо командой `/category_3`.
...
| `/category` | Сменить категорию подписки (нужная/на-вылет) |
| `/delete_НОМЕР` | Удалить подписку по номеру из списка (например `/delete_3`) |
| `/category_НОМЕР` | Сменить категорию по номеру из списка (например `/category_3`) |
